...
import org.apache.cassandra.hadoop.cafs.core.CaFileSystem;
...
public class CassandraFileSystemTest {
  ...
  private static CassandraFileSystem fs;

  @BeforeClass
  public static void setup() throws IOException, CassandraException, org.apache.hadoop.hive.cassandra.CassandraException {
    ...
    fs = new CassandraFileSystem();
    URI cafsURI = URI.create("cfs://localhost:9160/");

    Configuration c = new Configuration();
    fs.initialize(cafsURI, c);
    assertNotNull(fs);
  ...
  private void testFileSystem(boolean flush) throws Exception {
    ...
    //fs.initialize(URI.create("cfs://localhost:9160/"), new Configuration());
