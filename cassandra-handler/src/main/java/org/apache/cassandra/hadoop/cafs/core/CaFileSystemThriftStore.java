...
/**
 * CAFS schema (note that we are borrowing from the original design, but use a different namespace
 * to make sure there is no ambiguity.
 */
public class CaFileSystemThriftStore implements CaFileSystemStore {

  private final static Logger logger = LoggerFactory.getLogger(CaFileSystemThriftStore.class);
  private static final String keySpace = "cafs";
  private static String inodeDefaultCf = "inode";
  private static String sblockDefaultCf = "sblocks";

  // CAFS for archive use
  ...
  // these values can be overridden if the archive mode is set
  ...
  static final String cafsInodeDefaultFamily = "inode";
  static final String cafsSubBlockDefaultFamily = "sblocks";
  static final ColumnParent subBlockDefaultDataPath = new ColumnParent(cafsSubBlockDefaultFamily);

  // CFs for archive kind of storage
  private static final String cafsInodeArchiveFamily = "inode_archive";
  private static final String cafsSubBlockArchiveFamily = "sblocks_archive";

  static final QueryPath inodeArchiveQueryPath = new QueryPath(cafsInodeArchiveFamily, null, dataCol);
  static final ColumnParent subBlockArchiveDataPath = new ColumnParent(cafsSubBlockArchiveFamily);

  private ByteBuffer compressedData = null;
  private ByteBuffer uncompressedData = null;

  private StorageType storageTypeInUse = StorageType.CAFS_REGULAR;
  ...
  public CaFileSystemThriftStore() {
  }
  ...
  public void initialize(URI uri, Configuration conf) throws IOException {
    ...
    /* try {
      client = new CassandraProxyClient(host, port, true, false)
    ...
    logger.debug("Client session set for keyspace of [ {} ]", ks.name);
  ...
  public KsDef createKeySpace() throws IOException {
    // Stagger create time so nodes don't get confused
    try {
      Thread.sleep(new Random().nextInt(5000));

      KsDef cafsKs = checkKeyspace();

      if (cafsKs != null)
        return cafsKs;

      List<CfDef> cafs = new ArrayList<CfDef>();
      CfDef cf = new CfDef();
      ...
      cafsKs = new KsDef()
              ...
              .setDurable_writes(System.getProperty("cafs.replication", "1").equals("1") ? true : false)
              .setCf_defs(cafs);


      client.system_add_keyspace(cafsKs);
      waitForSchemaAgreement(client);

      logger.debug("Create new keyspace {}", cafsKs.name);
  ...
  private void initConsistencyLevels(KsDef ks, Configuration conf) {
    ...
    consistencyLevelWrite = ConsistencyLevel.valueOf("QUORUM");
  ...
  /**
   * Set to different set of Column Families if the archive location is selected.
   */
  private void initCFNames(URI uri) {
    ...
    if (isArchive(uri)) {
      ...
      storageTypeInUse = StorageType.CAFS_ARCHIVE;
    } else {
      // cafs:///
  ...
  private boolean isArchive(URI uri) {
    return uri.getScheme().startsWith("cafs-archive");
  }
  ...
  @Override
  public void storeSubBlock(UUID parentBlockUUID, SubBlock sblock,
    ...
    try {
      client.insert(
              parentBlockId,
              sblockParent,
              new Column().setName(uuidToByteBuffer(sblock.id))
                      .setValue(compressedData)
                      .setTimestamp(System.currentTimeMillis()), consistencyLevelWrite);
    } catch (Exception e) {
      throw new IOException(e);
    }
  ...
  public void storeINode(Path path, INode inode) throws IOException {
    ...
    Map<String, List<Mutation>> pathMutations =
  ...
  private LocalBlock getLocalSubBlock(String subBlockCFName, ByteBuffer blockId,
    ...
    for (SSTableReader sstable : sstables) {
      ...
      long position = sstable.getPosition(decoratedKey, Operator.EQ).position;
  ...
  public LocalOrRemoteBlock get_cfs_sblock(String callerHostName, ByteBuffer blockId, ByteBuffer sblockId,
    ...
    if (storageType == StorageType.CAFS_REGULAR) {
      return get_cfs_sblock(callerHostName, cafsSubBlockDefaultFamily, blockId, sblockId, offset,
              subBlockDefaultDataPath);
    } else {
      return get_cfs_sblock(callerHostName, cafsSubBlockArchiveFamily, blockId, sblockId, offset,
  ...
  @Override
  public InputStream retrieveBlock(Block block, long byteRangeStart) {
    return new CaSubBlockInputStream(this, block, byteRangeStart);
  }

  public InputStream retrieveSubBlock(Block block, SubBlock subBlock,
  ...
  private ByteBuffer getPathKey(Path path) {
    ByteBuffer bb = ByteBufferUtil.bytes(path.toUri().getPath());
    String s = FBUtilities.hashToBigInteger(bb).toString(16);
    return ByteBufferUtil.bytes(s);
  }

  private ColumnOrSuperColumn performGet(ByteBuffer key, ColumnPath cp, ConsistencyLevel cl)
          throws IOException {
  ...
  public INode retrieveINode(Path path) throws IOException {
    ...
    // prepare return
    InputStream is = ByteBufferUtil.inputStream(pathInfo.column.value);
    long ts = pathInfo.column.getTimestamp();
    return INode.deserialize(is, ts);
  }


  public void deleteINode(Path path) throws IOException {
    try {
      client.remove(getPathKey(path),
              inodePath,
  ...
  public void deleteSubBlocks(INode inode) throws IOException {
    ...
    try {
      ...
      for (UUID subBlocksKey : subBlockKeys) {
        client.remove(ByteBuffer.wrap(UUIDGen.decompose(subBlocksKey)),
                sblockPath, System.currentTimeMillis(),
                consistencyLevelWrite);
      }
    } catch (Exception e) {
      throw new IOException(e);
  ...
  public Set<Path> listDeepSubPaths(Path path) throws IOException {
    ...
    try {
      List<KeySlice> keys = client.get_indexed_slices(inodeParent,
              new IndexClause(indexExpressions, ByteBufferUtil.EMPTY_BYTE_BUFFER, 100000),
              pathPredicate, consistencyLevelRead);
  ...
  @Override
  public BlockLocation[] getBlockLocation(List<Block> blocks, long start,
