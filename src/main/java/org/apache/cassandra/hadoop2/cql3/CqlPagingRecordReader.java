 ...
 */
public class CqlPagingRecordReader extends
		RecordReader<Map<String, ByteBuffer>, Map<String, ByteBuffer>>
		implements
		org.apache.hadoop.mapred.RecordReader<Map<String, ByteBuffer>, Map<String, ByteBuffer>> {

	private static final Logger logger = LoggerFactory
			.getLogger(CqlPagingRecordReader.class);
	...
	public void initialize(InputSplit split, TaskAttemptContext context)
		...
		totalRowCount = (this.split.getLength() < Long.MAX_VALUE) ? (int) this.split
				.getLength() : ConfigHelper.getInputSplitSize(conf);
		cfName = ConfigHelper.getInputColumnFamily(conf);
		consistencyLevel = ConsistencyLevel.valueOf(ConfigHelper
				.getReadConsistencyLevel(conf));
		keyspace = ConfigHelper.getInputKeyspace(conf);
		columns = CqlConfigHelper.getInputcolumns(conf);
		userDefinedWhereClauses = CqlConfigHelper.getInputWhereClauses(conf);

		try {
			pageRowSize = Integer.parseInt(CqlConfigHelper
					.getInputPageRowSize(conf));
		...
		try {
			...
			client = CqlPagingInputFormat.createAuthenticatedClient(location,
					port, conf);

			// retrieve partition keys and cluster keys from
			// system.schema_columnfamilies table
			if (cluster == null) {
				String cassandraServer = this.getLocation();
				cluster = Cluster.builder().addContactPoint(cassandraServer)
						.build();
			}
			tableMetaData = cluster.getMetadata().getKeyspace(this.keyspace)
					.getTable(this.cfName);
			retrieveKeys(tableMetaData);
	...
	public boolean nextKeyValue() throws IOException {
		...
		try {
			...
			// throw it as IOException, so client can catch it and handle it at
			// client side
	...
	private String getLocation() {
		Collection<InetAddress> localAddresses = FBUtilities
	...
	// now.
	public boolean next(Map<String, ByteBuffer> keys,
			Map<String, ByteBuffer> value) throws IOException {
		if (nextKeyValue()) {
			value.clear();
	...
	private class RowIterator
			extends
			AbstractIterator<Pair<Map<String, ByteBuffer>, Map<String, ByteBuffer>>> {
		...
		private String partitionKeyString; // keys in <key1>, <key2>, <key3>
											// string format
		private String partitionKeyMarkers; // question marks in ? , ? , ?
											// format which matches the number
											// of keys
		...
		protected Pair<Map<String, ByteBuffer>, Map<String, ByteBuffer>> computeNext() {
			...
			for (Column column : row.columns) {
				String columnName = stringValue(ByteBuffer.wrap(column
						.getName()));
		...
		/**
		 * check whether start to read a new CF row by comparing the partition
		 * keys
		 */
		private boolean newRow(Map<String, ByteBuffer> keyColumns,
			...
			if (keyColumns.size() == 1) {
				rowKey = partitionBoundColumns.get(0).validator.toString();
			} else {
				Iterator<ByteBuffer> iter = keyColumns.values().iterator();
				for (BoundColumn column : partitionBoundColumns) {
					rowKey = rowKey + column.validator.getName() + ":";
				}
			}
		...
		/**
		 * set the last non-null key value to null, and return the previous
		 * index
		 */
		...
		/**
		 * serialize the prepared query, pair.left is query id, pair.right is
		 * query
		 */
		private Pair<Integer, String> composeQuery(String columns) {
			...
			if (columns == null) {
				...
				columns = (clusterKey == null || "".equals(clusterKey)) ? partitionKey
						+ "," + columns
			...
			return Pair.create(clause.left, String.format(
					"SELECT %s FROM %s%s%s LIMIT %d ALLOW FILTERING", columns,
					quote(cfName), clause.right, whereStr, pageRowSize));
		}
		...
		private String withoutKeyColumns(String columnString) {
			...
			for (BoundColumn column : Iterables.concat(partitionBoundColumns,
					clusterColumns)) {
		...
		private Pair<Integer, String> whereClause() {
			...
			if (emptyPartitionKeyValues()) {
				return Pair.create(0, String.format(
			...
			if (clusterColumns.size() == 0
					|| clusterColumns.get(0).value == null) {
				return Pair.create(1, String.format(
						" WHERE token(%s) > token(%s)  AND token(%s) <= ?",
						partitionKeyString, partitionKeyMarkers,
						partitionKeyString));
			}
			...
			return Pair.create(clause.left, String.format(
					" WHERE token(%s) = token(%s) %s", partitionKeyString,
					partitionKeyMarkers, clause.right));
		}
		...
		private Pair<Integer, String> whereClause(List<BoundColumn> column,
			...
			if (position == column.size() - 1
					|| column.get(position + 1).value == null) {
				return Pair.create(
						position + 2,
						String.format(" AND %s > ? ",
		...
		/**
		 * serialize the partition key string in format of <key1>, <key2>,
		 * <key3>
		 */
		...
		/**
		 * serialize the query binding variables, pair.left is query id,
		 * pair.right is the binding variables
		 */
		private Pair<Integer, List<ByteBuffer>> preparedQueryBindValues() {
			...
			// initial query token(k) >= start_token and token(k) <= end_token
			if (emptyPartitionKeyValues()) {
				values.add(partitioner.getTokenValidator().fromString(
						split.getStartToken()));
				values.add(partitioner.getTokenValidator().fromString(
						split.getEndToken()));
				...
				if (clusterColumns.size() == 0
					...
					// query token(k) = token(pre_partition_key) and m =
					// pre_cluster_key_m and n > pre_cluster_key_n
					int type = preparedQueryBindValues(clusterColumns, 0,
							values);
		 ...
		 */
		private int preparedQueryBindValues(List<BoundColumn> column,
				int position, List<ByteBuffer> bindValues) {
			if (position == column.size() - 1
					|| column.get(position + 1).value == null) {
		 ...
		 */
		private int prepareQuery(int type) throws InvalidRequestException,
				TException {
			...
			CqlPreparedResult cqlPreparedResult = client.prepare_cql3_query(
					ByteBufferUtil.bytes(query.right), Compression.NONE);
		...
		private void executeQuery() {
			...
			int retries = 0;
			// only try three times for TimedOutException and
			// UnavailableException
			while (retries < 3) {
				try {
					CqlResult cqlResult = client.execute_prepared_cql3_query(
							prepareQuery(bindValues.left), bindValues.right,
							consistencyLevel);
	...
	/**
	 * retrieve the partition keys and cluster keys from
	 * system.schema_columnfamilies table
	 */
	private void retrieveKeys(TableMetadata tableMetaData) throws Exception {
		...
		// CqlResult result =
		// client.execute_cql3_query(ByteBufferUtil.bytes(formatted),
		...
		// String keyString =
		// ByteBufferUtil.string(ByteBuffer.wrap("[\"row1\"]".getBytes()));
		// logger.debug("partition keys: {}", primaryKeys);
		// List<String> keys = FBUtilities.fromJsonList(keyString);

		for (ColumnMetadata primaryKey : partitionKeys) {
			partitionBoundColumns.add(new BoundColumn(primaryKey.getName()));
			partitionBoundColumns.get(0).validator = primaryKey.getType();
		}

		// keyString =
		// ByteBufferUtil.string(ByteBuffer.wrap(cqlRow.columns.get(1).getValue()));
		List<ColumnMetadata> clusterColumnsOfTable = tableMetaData
				.getClusteringColumns();
		// keyString = ByteBufferUtil.string(ByteBuffer.wrap("[]".getBytes()));
		// logger.debug("cluster columns: {}", keyString);
		// keys = FBUtilities.fromJsonList(keyString);

		for (ColumnMetadata clusterColumn : clusterColumnsOfTable) {
			clusterColumns.add(new BoundColumn(clusterColumn.getName()));
		}

		// Column rawKeyValidator = cqlRow.columns.get(2);
		// String validator =
		// ByteBufferUtil.string(ByteBuffer.wrap(rawKeyValidator.getValue()));
		// String validator = "org.apache.cassandra.db.marshal.UTF8Type";
	...
	/**
	 * get string from a ByteBuffer, catch the exception and throw it as runtime
	 * exception
	 */
