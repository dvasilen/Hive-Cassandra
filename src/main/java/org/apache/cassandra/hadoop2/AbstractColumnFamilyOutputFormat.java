...
public abstract class AbstractColumnFamilyOutputFormat<K, Y> extends OutputFormat<K, Y> implements org.apache.hadoop.mapred.OutputFormat<K, Y> {
    ...
    /**
     * Connects to the given server:port and returns a client based on the given
     * socket that points to the configured keyspace, and is logged in with the
    ...
    public static Cassandra.Client createAuthenticatedClient(String host, int port, Configuration conf) throws Exception {
        ...
        if (ConfigHelper.getOutputKeyspaceUserName(conf) != null) {
            ...
            creds.put(PasswordAuthenticator.USERNAME_KEY, ConfigHelper.getOutputKeyspaceUserName(conf));
            creds.put(PasswordAuthenticator.PASSWORD_KEY, ConfigHelper.getOutputKeyspacePassword(conf));
